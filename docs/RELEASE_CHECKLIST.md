...
- 普通对局已增加不被 Home 使用的隐藏 PVE 语音构造：只有权威状态给出真实 `humanPlayer`、处于玩家回合且用户点击用途说明后才创建 session；位置、完整移动和取消选择只进入既有 GameBloc 规则链，完整移动固定 `isAIMove=false`，AI/后台/终局会中断且不会自动重开麦克风。PVP、成功结果播报和普通对局控制动作仍未完成，不能标记为“普通对局语音完整”。
