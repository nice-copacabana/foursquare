...
- 现代东方隐藏页面已具备中英日用途说明、显式语音启用、TTS 完成后才启动开局时钟、单次聆听、暂停恢复、后台超时终局刷新、两段式弃局确认和窄屏可滚动布局；入口继续隐藏，页面没有初始化具体插件或新增平台权限。
- 基于当前锁定依赖的候选生产语音 Adapter 已完成懒加载与插件隔离：权限状态 fail-closed，STT 采用单次中文短指令并隔离迟到回调，TTS 只在真实 completion 后成功，cancel/error 可恢复重播，终止回调超时会熔断。平台权限和设置跳转、正式页面装配、真机音频与最终语音引擎隐私外部门禁仍未完成，当前不对用户开放。
