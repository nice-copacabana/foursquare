...
| 语音控制 + 冥想模式 | 历史文档曾宣称完成 | 隐藏的 Phase 4 核心、存储装配和现代东方冥想页面已实现；旧 Bloc/页面原型已移除；生产 Adapter、平台权限、正式入口和真机门禁未完成 | **历史宣称已纠正 ⚠️** |
| 回放 / 存储 / i18n / 主题 | ✅ 完成 | 三语 ARB 齐全、Hive 存档真实、3 套主题 presets 齐 | **一致 ✅** |
| **测试证据** | CHANGELOG 曾宣称未复测的覆盖率 | 当前共 71 个 `_test.dart` 文件；本次全量 `flutter test` 为 696 通过、1 跳过；未重新测量覆盖率 | **只采用当前命令证据 ⚠️** |
...
- `test/` 与 `integration_test/` 当前共有 71 个 `_test.dart` 文件。
- 本次全量 `flutter test`：696 项通过、1 项显式跳过。
...
3. **Phase 4 产品化**：生产 ASR/TTS Adapter、平台权限与设置跳转、正式入口、真机音频/隐私验收尚未完成；隐藏页面、无屏整局和持久化编排已有自动化证据。
...
| Phase 4 语音与冥想 | 隐藏核心与现代东方页面已形成无屏整局、独立存档和自动收口证据；生产 Adapter、平台权限、正式入口和真机门禁未完成 |
...
- Phase 4：继续完成生产 ASR/TTS Adapter、平台权限与设置跳转、正式入口和真机音频/隐私验收。
