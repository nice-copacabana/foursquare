...
当前已完成隐藏的普通语音纯核心和权威冥想 session，并用假端口跑通 15 ply 自然终局的无屏整局；运行时已串联自动保存、恢复和终局归档，现代东方隐藏页面已通过可替换端口接入用途说明、开场、单次聆听和触屏控制。这些是本地自动化证据，尚未达到用户可用或商店可发布状态。首期不承诺英文、日文、手表或手环适配，也不在外部引擎确定前开放麦克风入口。
