...
> 当前状态：Phase 1 Android 正式版开发中，尚未达到正式发布门禁。仓库中的历史完成报告和旧版本号不代表当前可发布状态。
> 当前范围与顺序以[产品路线图](docs/PRODUCT_ROADMAP.md)为准，规则以[四方棋规则与对局协议](docs/GAME_RULES.md)为准，文档有效性见[文档状态索引](docs/DOCUMENT_STATUS.md)。

## 项目概述

棋盘为 4×4 交点棋盘，黑白双方各有 4 枚棋子。玩家每次将一枚棋子移动到上下左右相邻的空位，并通过精确的四格排列触发横向或纵向吃子。完整吃子模式、50 ply 和棋、超时以及终局优先级见权威规则文档。
...
这些是 Phase 1 的交付目标，不表示当前代码已经全部完成。上线前必须通过[发布门禁清单](docs/RELEASE_CHECKLIST.md)和[测试策略](docs/TEST_STRATEGY.md)。
...
Phase 2 才建立和发布 iOS 工程，最低系统版本为 iOS 13。
...
仓库已经包含本地双人、AI、LAN、存储、统计、回放、主题、音频、在线原型以及语音/冥想原型代码，但“存在代码”不等于“达到正式发布质量”。当前正在按 Phase 1 权威规则统一引擎、BLoC、AI、存档、回放和 LAN 协议，并重做现代东方视觉体系。

当前不能作出的发布承诺包括：

- 不能把旧规则测试通过视为新权威规则已经完整验证。
- 不能把已有 LAN 移动收发视为计时、断线重连和权威同步已经完成。
- 不能宣称在线后端、在线客户端、普通语音或冥想模式可用于 Phase 1。
...
- Phase 1 仍需补齐完整离线对局、生命周期、双端 LAN、断线重连和发布包真机验收。
