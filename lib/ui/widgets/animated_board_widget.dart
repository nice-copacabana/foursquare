...
class _AnimatedBoardWidgetState extends State<AnimatedBoardWidget>
  ...
  Map<Position, PieceType> _capturingPieces = const {};
  ...
  void didUpdateWidget(AnimatedBoardWidget oldWidget) {
    ...
    if (lastMoveChanged ||
        !listEquals(
          widget.capturedPiecePositions,
          oldWidget.capturedPiecePositions,
        )) {
      if (widget.capturedPiecePositions.isEmpty) {
        _captureGeneration++;
  ...
  /// 播放吃子动画
  void _playCaptureAnimation(
    List<Position> positions,
    BoardState previousBoard,
  ) {
    if (!widget.animationEnabled) return;

    final pieces = <Position, PieceType>{};
    for (final position in positions) {
      final piece = previousBoard.getPiece(position);
      ...
      child: Stack(
        children: [
          ...
          if (widget.animationEnabled &&
              _animatingPiece != null &&
              _moveAnimation != null)
            _buildAnimatingPiece(cellSize),

          // Captured piece
          if (widget.animationEnabled)
            ..._capturingPieces.entries.map(
              (capture) => _buildCapturingPiece(
                cellSize,
                capture.key,
                capture.value,
              ),
          ...
          if (widget.animationEnabled &&
              ...
              _capturingPieces.isNotEmpty)
            ..._capturingPieces.entries.map(
              (capture) => RepaintBoundary(
                child: _buildCaptureParticles(
                  boardSize,
                  cellSize,
                  capture.key,
                  capture.value,
  ...
  /// 构建被吃的棋子（带消失动画）
  Widget _buildCapturingPiece(
    double cellSize,
    Position position,
    PieceType piece,
  ) {
    return AnimatedBuilder(
      key: ValueKey('captured-piece-${position.x}-${position.y}'),
