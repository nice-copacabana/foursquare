...
class GameBloc extends Bloc<GameEvent, GameState> {
  ...
  Future<void> _onRestartGame(
    ...
    final currentAIDifficulty =
        state is GamePlaying ? (state as GamePlaying).aiDifficulty : null;
  ...
  Future<void> _onMovePiece(
    ...
    // 执行移动
    final result =
        _gameEngine.executeMove(playing.boardState, event.from, event.to);
    ...
    if (playing.mode == GameMode.pve &&
        result.newBoard!.currentPlayer == PieceType.white) {
      add(const AIPlayEvent());
    }
  ...
  Future<void> _onSaveGame(SaveGameEvent event, Emitter<GameState> emit) async {
    ...
    try {
      final gameSave = GameSave(
        ...
        moveHistory:
            playing.moveHistory.map((m) => MoveData.fromMove(m)).toList(),
        currentPlayer:
  ...
  Future<void> _updateStatistics(
    ...
    final isWin = (gameResult.status == GameStatus.blackWin &&
            playing.mode == GameMode.pvp) ||
        (gameResult.status == GameStatus.blackWin &&
            playing.mode == GameMode.pve);
    final isLoss = (gameResult.status == GameStatus.whiteWin &&
        playing.mode == GameMode.pve);
    final isDraw = gameResult.status == GameStatus.draw;

    // 计算吃子数
    final captures =
        playing.moveHistory.where((m) => m.capturedPiece != null).length;
